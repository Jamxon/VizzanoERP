        ...
        foreach ($employees as $employee) {
            ...
            foreach ($empDataPerGroup as $gid => $row) {
                ...
                $monthlyPieceworkData = $mp ? [
                    'id' => $mp->id, 'amount' => (float)$mp->amount, 'status' => (bool)$mp->status,
                    'created_by' => $mp->created_by_name, 'comment' => $mp->comment
                ] : null;

                $ms = $monthlySalariesData[$employee->id] ?? null;
                $monthlySalaryData = $ms ? [
                    'id' => $ms->id, 'amount' => (float)$ms->amount, 'status' => (bool)$ms->status,
                    'created_by' => $ms->created_by_name, 'comment' => $ms->comment
                ] : null;

                // ✨ Yangi qo‘shimcha to’lov ma’lumotlari
                $paymentsList = $payments[$employee->id] ?? collect();
                $paidTotal = $paymentsList->sum('amount');
                $paidAmountsByType = $paymentsList->groupBy('type')->map(fn($t) => round($t->sum('amount'), 2));
                ...
                $processed[$gid][] = [
                    ...
                    /** 🆕 Qo‘shimcha kiritilgan 4 ta maydon **/
